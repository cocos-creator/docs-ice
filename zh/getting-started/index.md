...
- 下载安装。访问 [官方下载地址](链接地址)，下载 Cocos ICE（下面简称 ICE）安装包，运行安装包，完成ICE安装；
...
> **注意**：Cocos ICE 的使用前提是获取 [License](../License/index-1.md)，若存疑请咨询商务（商务的联系方式）
...
您可以通过访问 Cocos 产品首页，下载 Cocos ICE 的安装包。
...
3. **搜索** 与 **同步**。支持在当前目录下进行模糊搜索，点击 ![刷新](img/刷新.png)，可 **刷新** 当前课程广场。
...
2. 舞台中的所有对象，都会在 **对象列表** 中出现。选中对象后，可在对应的 **属性面板** 中修改显示样式、位置。
...
接下来，我们来看一下通用工具，主要围绕 **基础对象**、**素材库**、**系统工具**、**操作快捷键** 这 5 个模块展开。
...
鼠标 hover 对象。此时单击鼠标，就会选中该对象。选中对象后可通过 **属性** 面板进行修改。

#### 移动对象

鼠标 **长按** 所选中对象，可对对象进行拖拽移动，支持拖动约束框对对象进行缩放。
...
点击工具栏中的素材库，可快速唤出 **素材库** 弹窗，通过 Tap 可切换素材类型，支持预览素材。选中素材后点击插入，素材将置入场景编辑器中。
...
详情请参看上文 [互动视频制作](#%E4%BA%92%E5%8A%A8%E8%A7%86%E9%A2%91%E6%A8%A1%E5%BC%8F) 部分的内容。
...
展示当前场景编辑器中所引用的所有对象、组件（由对象组成）。可对选中对象进行 **编辑**（选中后，可通过属性面板、事件面板进行配置）和 **删除** 操作；

#### 基础操作

在场景编辑器中添加的内容，都会出现在对象列表中，对象若是集合，则将以树状结构进行展示。**右键** 对象将唤出更多操作（见下图）。
